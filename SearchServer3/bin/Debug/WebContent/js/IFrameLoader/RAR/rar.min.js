(function() {
    ...
    Reader.prototype.readFile = function(length, position, callback) {
        var slice = this.file.slice(position, position + length), fr = new FileReader;
    ...
    DataView.prototype.getStringUtf16 = function(length, offset, bom) {
        offset = offset || 0;
        length = length || this.byteLength - offset;
        var littleEndian = false, str = "", useBuffer = false;
        if (typeof Buffer !== "undefined") {
            str = [];
            useBuffer = true
        }
    ...
    var RarArchive = function() {
        ...
        if (!this.options.file) {
            if (arguments[1])
                arguments[1].call(this, "No file was set");
            return
        }
        if (this.options.type === RarArchive.OPEN_FILE) {
            if (typeof window === "undefined" || !window.File || !window.FileReader || typeof ArrayBuffer === "undefined") {
                if (arguments[1])
                    arguments[1].call(this, "Browser does not have support for the File API and/or ArrayBuffers")
            }
        } else if (this.options.type === RarArchive.OPEN_LOCAL) {
            if (typeof require !== "function") {
                if (arguments[1])
                    arguments[1].call(this, "Local paths may not be read within a browser");
                return
            }
        } else {
        }
        ...
        this.rd.open(this.file, function(err) {
            ...
            self.rd.read(14, 0, function(err, data) {
                ...
                if (view.getString(7, 0, true) !== "Rar!\x00") {
                    if (callback)
                        callback.call(self, "Invalid RAR archive");
                    return
                }
                var crc = view.getUint16(7, true), type = view.getUint8(9), flags = view.getUint16(10, true), size = view.getUint16(12, true);
                if (type !== 115) {
                    if (callback)
                        callback.call(self, "Invalid RAR archive");
                    return
                ...
                if ((flags & 128) !== 0) {
                    if (callback)
                        callback.call(self, "Encrypted RAR archives are not yet supported");
                    return
                }
                self.valid = true;
    ...
    RarArchive.prototype.parseEntry = function(offset, callback) {
        ...
        this.rd.read(11, offset, function(err, data) {
            ...
            var view = new DataView(data), flags = view.getUint16(3, true), size = view.getUint16(5, true);
    ...
    RarArchive.prototype.readHeaders = function(offset, callback) {
        ...
        var cb = function(err, data) {
            ...
            var view = new DataView(data), crc = view.getUint16(0, true), type = view.getUint8(2), flags = view.getUint16(3, true), size = view.getUint16(5, true);
            if ((flags & 32768) !== 0) {
            ...
            switch (type) {
                case 116:
                    self.parseEntry(offset, function(err, entry) {
                        if (err) {
                            if (callback)
                                callback.call(self, "Read error occurred while reading entry");
                            return
                        }
        ...
        if (this.rd.size <= 14) {
            if (callback)
                callback.call(self, null, this.entries);
            return
        }
    ...
    RarArchive.prototype.get = function(entry, callback) {
        if (!this.valid) {
            if (callback)
                callback.call(this, "Invalid RAR archive");
            return
        }
        if (!(entry instanceof RarEntry)) {
            if (callback)
                callback.call(this, "Invalid RAR entry, must be an instance of RarEntry");
            return
        }
        if (entry.method !== RarEntry.METHOD_STORE) {
            if (callback)
                callback.call(this, "Compression is not yet supported");
            return
        }
